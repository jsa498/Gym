...
export function Auth() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to the setup page with force=true to ensure complete setup
      if (data.user) {
        // Force redirect to go through the setup page which will check if setup is complete
        router.push(`/setup?userId=${data.user.id}&force=true`);
      } else {
        router.push('/');
      }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white p-4">
      <div className="w-full max-w-sm space-y-6 relative">
        ...
        <div className="text-center text-sm">
          ...
          <Link href="/auth/signup" className="text-white hover:text-blue-400 font-medium transition-colors duration-200 hover:brightness-125">
            Sign up
          </Link>
        </div>
      </div>
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // Force the template_preference to be null to ensure setup flow
        await supabase
          .from('profiles')
